...
// /api/user/registry
router.post("/registy",bodyParser.json(),sess,async(req,res)=>{
    ...
    try{
        if(count == 0) {
            const{
                email,telephone,password,nickname,avatarurl
            }=result.data;
...
router.put("/information", bodyParser.json(), sess, async (req, res) => {
